...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (nevents < epcf->events)
    {
        ...
        // 创建event_list数组，用于进行epoll_wait调用时传递内核态事件
        // event_list数组就是用于在epoll_wait调用中接收事件的参数
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    // 根据event参数确定是读事件还是写事件，决定event时添加EPOLLIN标志还是EPOLLOUT标志
...
// ngx_epoll_process_events方法会收集当前触发的所有事件，对于不需要延后处理的事件，立即调用回调方法。
// 每个回调方法都不能导致进程休眠或者消耗太多的时间，以免epoll不能立即处理其他事件。
static ngx_int_t
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    //遍历所有epoll_wait返回的所有事件
    for (i = 0; i < events; i++)
    {
        // ngx_connection_t对象指针
        c = event_list[i].data.ptr;

        // 转换成ngx_connection_t对象
        ...
        if ((revents & EPOLLIN) && rev->active)
        {
            ...
            // 含有NGX_POSt_EVENTS标识，表示该事件要延后处理
