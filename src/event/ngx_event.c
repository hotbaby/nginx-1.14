...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    // 预分配ngx_connection_t数组，数组个数为nginx.conf配置文件中connections或worker_connections配置的连接数
    cycle->connections =
    ...
    // 预分配ngx_event_t事件数组作为读事件池，数组大小为nginx.conf配置worker_connections配置的连接数量
    ...
    // c = cycle->connections;
    // 将读、写事件配置到ngx_connection_t连接对象中
    do
    {
        i--;
    ...
    // free_connections指向connections数组的最后一个元素
    cycle->free_connections = next;
    cycle->free_connection_n = cycle->connection_n;

    /* for each listening socket */

    // 为所有ngx_listen_t监听对象中connection成员分配连接
...
#else
        // 同时对监听端口的读事件设置处理方法为ngx_event_accpt
        // 也就是说有新连接事件时，将调用ngx_event_accept方法建立新连接
