...
// 当Nginx接收到用户发起TCP连接的请求时，事件框架将会负责把TCP连接建立起来，
// 如果TCP连接成功建立，HTTP框架就会介入请求的处理了，
// 在ngx_event_accept 方法建立新连接的最后1步，将会调用ngx_listening_t监听结构体的handler方法。
// HTTP框架处理请求的第一步就在ngx_http_init_connection方法中，这里传入的参数c就 是新建立的连接。
...
// 本节将描述接收HTTP头部这一阶段，该阶段是通过ngx_http_process_request_headers方法实现的，
// 该方法将被设置为连接的读事件回调方法，在接收较大的HTTP头部时，它有可 能会被反复多次地调用。
...
// ngx_http_free_request方法将会释放请求对应的ngx_http_request_t数据结构，
// 它并不会像 ngx_http_close_connection方法一样去释放承载请求的TCP连接，
// 每一个TCP连接可以反复地承载多个HTTP请求，
// 因此，ngx_http_free_request是比ngx_http_close_connection更高层次的方法，前者必然先于后者调用。
...
// ngx_http_close_connection方法是HTTP框架提供的一个用于释放TCP连接的方法，
// 它的目的很简单，就是关闭这个TCP连接，当且仅当HTTP请求真正结束时才会调用这个方法。
